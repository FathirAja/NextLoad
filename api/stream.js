...
const { classifyYtDlpError } = require("./lib/ytdlp");
...
module.exports = async function handler(req, res) {
  ...
  if (!rateLimit(`stream:${ip}`, 5, 60_000)) {
    return res
      ...
      .json({
        success: false,
        error: "Too many download requests. Wait 1 minute.",
      });
  }
  ...
  if (!formatId)
    return res.status(400).json({ success: false, error: "Missing formatId." });
  ...
  res.setHeader(
    "Content-Type",
    mimeMap[outputFormat] || "application/octet-stream",
  );
  res.setHeader(
    "Content-Disposition",
  ...
  child.on("close", (code) => {
    ...
    if (code !== 0 && !headersSent) {
      ...
      const classified = classifyYtDlpError({ message: stderrBuf });
      const errMsg =
        classified?.error ||
        stderrBuf.slice(0, 200) ||
        `yt-dlp exited with code ${code}`;
      const status = classified?.status || 500;
